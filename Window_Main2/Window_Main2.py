...
from PyQt5.QtWidgets import QMainWindow, QWidget, QComboBox, QFrame, QLabel, QGroupBox, QToolButton, QStatusBar, QApplication, QRadioButton
...
form_class = uic.loadUiType("GUI/mainWindow2.ui")[0]
...
# choice of combo_analysis changes the content of combo_dataset2
    def dataset_type(self):
        ...
        if self.combo_analysis.currentText() == "Survival plots":
            ...
            self.combo_query.addItems(["(Query data)", "Gene expression"])
            self.combo_target.clear()
            self.combo_target.addItems(["(Target data)","Patient Survival"])
        elif self.combo_analysis.currentText() == "Cross-association":
            self.combo_query.clear()
            self.combo_query.addItems(["(Query data)", "Genes expression", "Drugs screen", "sgRNAs screen"])
            self.combo_target.clear()
            self.combo_target.addItems(["(Target data)", "Gene expression", "Drug screen", "sgRNA screen"])
        elif self.combo_analysis.currentText() == "Scatter 2D/3D plots":
            self.combo_query.clear()
            self.combo_query.addItems(["(Query data)", "Gene expression", "Drug screen", "sgRNA screen"])
            self.combo_target.clear()
            self.combo_target.addItems(["(Target data)", "Gene expression", "Drug screen", "sgRNA screen"])
