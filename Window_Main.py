...
form_class = uic.loadUiType("GUI/mainWindow(2).ui")[0]
currentPath = os.getcwd()
...
# comboboxes of 'Create project'
    def initUI(self):
        self.check_patient.stateChanged.connect(self.analysis_type)
        self.check_cell.stateChanged.connect(self.analysis_type)

        self.combo_analysis.currentIndexChanged.connect(self.dataset_type)

        self.kmplot_btn.clicked.connect(self.openSurvival)
        self.cross_btn.clicked.connect(lambda: self.openCRISPR("sgRNA", "genes"))
        self.sample_btn.clicked.connect(lambda: self.open2Dist("Gene", "Drug"))

        self.create_btn.clicked.connect(self.openWindow)

        self.statusBar().showMessage('CBiS')

    def analysis_type(self):
        if self.check_patient.isChecked() == True:
            self.combo_analysis.clear()
            self.combo_analysis.addItems(["(Select analysis)", "Survival plots"])
        elif self.check_cell.isChecked() == True:
            self.combo_analysis.clear()
            self.combo_analysis.addItems(["(Select analysis)", "Cross-association", "Scatter 2D/3D plots", "Box plot"])
...
# choice of combo_analysis changes the content of combo_dataset2
    def dataset_type(self):
        ...
        if self.combo_analysis.currentText() == "Survival plots":
            self.combo_dataset1.clear()
            self.combo_dataset1.addItems(["(Dataset1)", "Gene expression"])
            self.combo_dataset2.clear()
            self.combo_dataset2.addItems(["(Dataset2)","Patient Survival"])
        elif self.combo_analysis.currentText() == "Cross-association":
            self.combo_dataset1.clear()
            self.combo_dataset1.addItems(["(Dataset1)", "Genes expression", "Drugs screen", "sgRNAs screen"])
            self.combo_dataset2.clear()
            self.combo_dataset2.addItems(["(Dataset2)", "Gene expression", "Drug screen", "sgRNA screen"])
        elif self.combo_analysis.currentText() == "Scatter 2D/3D plots":
            self.combo_dataset1.clear()
            self.combo_dataset1.addItems(["(Dataset1)", "Gene expression", "Drug screen", "sgRNA screen"])
            self.combo_dataset2.clear()
            self.combo_dataset2.addItems(["(Dataset2)", "Gene expression", "Drug screen", "sgRNA screen"])
        elif self.combo_analysis.currentText() == "Box plot":
            self.combo_dataset1.clear()
...
# connect windows according to combo_analysis
    def openWindow(self):
        ...
        if self.combo_dataset2.currentText() == "Patient Survival":
            self.openSurvival()
        ...
        # cross-analysis
            ...
            # combo_dataset1
            if self.combo_dataset2.currentText() == "Gene expression":
                data1 = "gene"
            elif self.combo_dataset2.currentText() == "Drug screen":
                data1 = "drug"
            elif self.combo_dataset2.currentText() == "sgRNA screen":
                data1 = "sgRNA"
            # combo_dataset2
            if self.combo_dataset1.currentText() == "Genes expression":
                data2 = "genes"
            elif self.combo_dataset1.currentText() == "Drugs screen":
                data2 = "drugs"
            elif self.combo_dataset1.currentText() == "sgRNAs screen":
                data2 = "sgRNAs"
            self.openCRISPR(data2, data1)  # (d,c) = query, target(~s)

        elif self.combo_analysis.currentText() == "Scatter 2D/3D plots":
            # combo_dataset1
            if self.combo_dataset1.currentText() == "Gene expression":
                data1 = "Gene"
            elif self.combo_dataset1.currentText() == "Drug screen":
                data1 = "Drug"
            elif self.combo_dataset1.currentText() == "sgRNA screen":
                data1 = "sgRNA"
            # combo_dataset2
            if self.combo_dataset2.currentText() == "Gene expression":
                data2 = "Gene"
